...
// ── Layout mobile: cards en columna estática ──────────────────────────────────
const MobileLayout = () => (
  <div className="bg-[#000A15] px-4 py-12 flex flex-col gap-6">
    {cards.map((card) => (
      <div
        key={card.title}
        className="bg-[#D9D9D9]/25 backdrop-blur-[38.8px] border border-white/10 overflow-hidden"
      >
...
// ── Layout desktop: funcionalidad sticky scroll original ──────────────────────
const DesktopLayout = () => {
...
// ── Componente principal: decide qué layout renderizar ────────────────────────
...
const CardContent = ({ card }: { card: typeof cards[0] }) => (
  <div className="grid grid-cols-1 lg:grid-cols-10 gap-6 lg:gap-12 p-6 lg:p-12 w-full h-full items-center">
    <div className="lg:col-span-4 w-full h-48 lg:h-full lg:max-h-full bg-gray-800/50 border border-white/5 overflow-hidden shadow-inner">
      <img
        src={card.img}
    ...
    <div className="lg:col-span-6 flex flex-col justify-center text-left">
      <h2 className="text-white text-2xl lg:text-4xl mb-3 lg:mb-4" style={{ fontFamily: "var(--font-michroma)" }}>
        {card.title}
      </h2>
      <div className="flex gap-6 mb-4 lg:mb-6 text-white/90 tracking-[0.2em] text-sm font-medium">
        {card.tags.map((tag) => (
          <span key={tag} style={{ fontFamily: "var(--font-rajdhani)" }}>{tag}</span>
        ))}
      </div>
      <p
        className="text-white/70 text-base leading-relaxed mb-6 lg:mb-10 max-w-3xl font-light"
