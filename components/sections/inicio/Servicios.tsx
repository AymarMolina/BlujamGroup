...
export default function NuestrosServicios() {
  ...
  return (
    <section ref={container} className="bg-[#000A15] text-white px-20 pt-20 min-h-screen overflow-hidden">
      <div className="gsap-section-content">
        
        <div className="mb-16">
          <h2 className="flex items-center gap-0 text-[2rem] font-black tracking-[0.12em] uppercase leading-none" style={{ fontFamily: "var(--font-michroma)" }}>
            <span>NUESTROS</span>
            <span className="bg-[#e85d04] text-white px-3 py-1 inline-block ml-2" style={{ clipPath: "polygon(6px 0%, 100% 0%, calc(100% - 6px) 100%, 0% 100%)" }}>SERVICIOS</span>
          </h2>
          <p className="text-white/90 text-xl mt-3 font-normal tracking-wide" style={{ fontFamily: "Rajdhani, sans-serif" }}>
            Un ecosistema de capacidades tecnológicas para potenciar cada pilar de su negocio.
          </p>
        </div>

        <div className="grid grid-cols-[1fr_auto_1fr] gap-x-0 items-start relative">
              ...
              {service.title.split(" ").map((word, i) => (
                <span key={`word-${i}`} className="inline-block whitespace-nowrap">
                  {word.split("").map((char, j) => (
                    <span key={`char-${j}`} className="char inline-block">
                      {char}
                    </span>
                  ))}
          ...
          <div className="image-container relative p-6" style={{ width: "420px", height: "590px" }}>
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none z-20" viewBox="0 0 420 590">
              ...
              <path 
                ...
                d="M340 10 H400 Q410 10 410 20 V120" 
                stroke="#e85d04" strokeWidth="4" fill="none" strokeLinecap="round" 
              />
              
              <path 
                className="corner-path" 
                d="M80 580 H20 Q10 580 10 570 V470" 
                stroke="#e85d04" strokeWidth="4" fill="none" strokeLinecap="round" 
              />
            </svg>
            ...
            <div className="w-full h-full overflow-hidden rounded-[4px] border border-white/10 bg-[#0a1118] relative z-1 flex items-center justify-center">
              ...
              <img 
                ...
                onLoad={(e) => {
                  gsap.fromTo(e.currentTarget, { opacity: 0 }, { opacity: 1, duration: 0.5 });
                }}
              />
            </div>
          </div>
          ...
          <div className="flex flex-col h-[550px] pl-10 justify-between">
            ...
            <div className="mt-auto">
              <div className="text-sm mb-2" style={{ fontFamily: "sans-serif" }}>
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  disabled={current === 0} 
                  className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-[#e85d04] hover:border-[#e85d04] transition-all disabled:opacity-10 cursor-pointer"
                >
                  ←
                </button>
                <button 
                  onClick={() => setCurrent((p) => Math.min(services.length - 1, p + 1))} 
